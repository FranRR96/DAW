...
#numeros = seran las listas y num = seran los numeros introducidos
...
#En la función suma, agregué un parámetro numeros2 para recibir la lista de números que se obtiene de la función obtener_sumatorio. 
...
lista_1 = [1,3,5,8,545,99,665,4434,4456]
...
def nombre_Letra_Comienzo (nombre, letra):
    nombres_Lista= []
    for i in nombre:
        if i[0].lower() == letra.lower():
            nombres_Lista.append(i)
    return nombres_Lista

lista_Nombres=["Fran", "Franklin", "Francisca", "Pablo", "Sofia"]

nombre_Comienzo= "F"

print(nombre_Letra_Comienzo(lista_Nombres,nombre_Comienzo))
...
def cadena_Mas_Larga(lista_A,lista_B):
    if len(lista_A) > len(lista_B):
        return lista_A
    elif len(lista_A) < len(lista_B):
        return lista_B
    elif len(lista_A) == len(lista_B):
        contadorCaracteresRepetidos_A=0
        contadorCaracteresRepetidos_B=0
        for i in lista_A:
            if i == lista_A[0]:
                contadorCaracteresRepetidos_A+=1
        for i in lista_B:
            if i == lista_B[0]:
                contadorCaracteresRepetidos_B+=1
        if contadorCaracteresRepetidos_A > contadorCaracteresRepetidos_B:
            return lista_A
        else:
            return lista_B
...
lista_A=[1,1,3,1]
lista_B=[1,5,6,7]
print(cadena_Mas_Larga(lista_A, lista_B))
